"""
LearnFlow state machine — adaptive learning graph.
Manages session state and node functions for the personalised learning workflow.
"""
import json
import re
import time
...
from api.endpoits import search_web, edu_resources, search_youtube
...
def node_evaluate_quiz(state: LearnFlowState) -> dict:
    """Returns evaluation result dict (score, passed, feedback, videos, edu_resources)."""
    module   = state.current_module_info()
    qa_pairs = "\n".join(
