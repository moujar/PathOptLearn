...
from config import GROQ_API_KEY, GROQ_MODEL, COLAB_API_URL

st.set_page_config(page_title="Learning — PathOptLearn", page_icon="🎓", layout="wide")
...
def groq_generate_questions(transcript_text, num_questions=5):
    ...
    all_questions = []
    for chunk, n_q in zip(chunks, questions_per_chunk):
        for _ in range(n_q):
...
The "correct" field is the 0-based index of the correct answer."""
    ...
    if not all_questions:
        return [], "No valid questions generated. Try again."
...
# ── COLAB API (optional external endpoint) ───────────────────
def call_generate_api(transcript_text, num_questions, temperature):
    endpoint = COLAB_API_URL.rstrip("/") + "/generate"
    try:
        resp = requests.post(
            endpoint,
            json={"transcript": transcript_text, "num_questions": num_questions, "temperature": temperature},
            timeout=300
        )
        resp.raise_for_status()
...
def call_evaluate_api(transcript_text, questions, student_answers, preference):
    endpoint = COLAB_API_URL.rstrip("/") + "/evaluate"
    try:
        resp = requests.post(
            endpoint,
            json={"transcript": transcript_text, "questions": questions,
                  "student_answers": student_answers, "preference": preference},
            timeout=300
        )
        resp.raise_for_status()
...
def generate_questions(transcript_text, num_questions, temperature):
    if st.session_state.get("provider", "groq") == "groq":
...
# ── YOUTUBE SEARCH FOR RECOMMENDATIONS ──────────────────────
def search_youtube_candidates(query, max_results=10):
    candidates = []
    ydl_opts = {
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for entry in results.get('entries', []):
                ...
                snippet = (
                    " ".join([t['text'] for t in transcript])[:1500]
                ...
                candidates.append({
                    ...
                    "has_captions": has_captions, "snippet": snippet,
                })
    except Exception as e:
...
Answer with ONLY 3-5 words, nothing else. Be specific.
...
\"\"\"{current_transcript[:600]}\"\"\""""
    try:
        ...
        resp.raise_for_status()
        return resp.json()["choices"][0]["message"]["content"].strip().strip('"').lower()
    except Exception:
        return None
...
def build_search_query(current_transcript, eval_result, questions, user_answers):
    ...
    for q, student_ans in zip(questions, user_answers):
        ...
        if student_ans != correct_ans:
            weak_concepts.append(" ".join(q['question'].split()[:6]))

    query = f"{topic} advanced" if eval_result["passed"] else f"{topic} beginner explained"
...
Use the exact title from the list above."""
    ...
    try:
        ...
        resp.raise_for_status()
        content = re.sub(r"```json|```", "", resp.json()["choices"][0]["message"]["content"].strip()).strip()
        ...
        for c in candidates:
            if any(kw in c["title"].lower() for kw in topic_keywords) and c["has_captions"]:
                return {"video_id": c["video_id"], "title": c["title"], "reason": "Best on-topic match with captions."}
        return {"video_id": candidates[0]["video_id"], "title": candidates[0]["title"], "reason": "Best available match."} if candidates else None
...
def recommend_next_video(current_transcript, eval_result, preference, questions, user_answers):
    ...
    with st.spinner("🎬 Searching YouTube..."):
        candidates = search_youtube_candidates(query, max_results=10)
    ...
    with st.spinner("🤖 Picking the best video..."):
        return groq_pick_best_video(candidates, eval_result, preference, topic)
...
with st.sidebar:
    ...
    provider_label = st.selectbox(
        "Model provider",
        options=["Groq (Cloud — Free)", "Custom API (Colab)"],
        index=0,
    ...
    if st.session_state["provider"] == "groq":
        ...
        st.caption("Using custom model via Colab.")
    st.markdown("---")
...
if st.session_state.get("questions"):
    ...
    if submitted:
        ...
        with st.spinner("Evaluating your performance..."):
            eval_result, error = evaluate_answers(full_text, questions, user_answers, preference)
